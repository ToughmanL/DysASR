...
class CustomDataset(Dataset):
    ...
    def _get_list_data(self, idx):
        ...
        key = data_list["key"] if 'key' in data_list.keys() else None
        ...
        return sample, sample_rate, transcript, language, key
    ...
    def __getitem__(self, idx):
        try:
            # 从数据列表里面获取音频数据、采样率和文本
            sample, sample_rate, transcript, language, key = self._get_list_data(idx=idx)
            # 可以为单独数据设置语言
            ...
            data['key'] = key if key is not None else str(idx)
            return data
